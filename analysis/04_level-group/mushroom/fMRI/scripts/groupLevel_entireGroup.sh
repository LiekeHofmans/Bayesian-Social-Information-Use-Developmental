...
# This analysis was not used any further, because significant group differences were not visible across both groups, which is a bit confusing for visualization. 
...
if [[ $do_ownCertaintyE1_masked == 1 ]]; then 
    feat $dir_designFiles/masked_entireGroup_ownCertaintyE1.fsf
    now=`date`     
    echo "===> feat for own certainty has finished at $now" 
fi

if [[ $do_ownCertaintySI_masked == 1 ]]; then 
    feat $dir_designFiles/masked_entireGroup_ownCertaintySI.fsf
    now=`date`     
    echo "===> feat for own certainty has finished at $now" 
fi

if [[ $do_peerConfidence_masked == 1 ]]; then 
    feat $dir_designFiles/masked_entireGroup_peerConfidence.fsf
    now=`date`   
