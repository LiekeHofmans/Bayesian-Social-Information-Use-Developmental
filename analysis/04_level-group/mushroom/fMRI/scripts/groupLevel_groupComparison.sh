...
do_ownCertaintyE1_masked=0 # run group level for certainty (from certaintyE1_confidence model)
do_peerConfidence_masked=0 # run group level for confidence averaged across low and high certainty conditions (from certaintyE1_confidence model)
do_ownCertaintySI_masked=0 # run group level for certainty (from certaintySI_confidence model)
...
if [[ $do_ownCertaintyE1_masked == 1 ]]; then 
    feat $dir_designFiles/masked_groupComparison_ownCertaintyE1.fsf
    now=`date`     
    echo "===> feat for own certainty has finished at $now" 
fi

if [[ $do_ownCertaintySI_masked == 1 ]]; then 
    feat $dir_designFiles/masked_groupComparison_ownCertaintySI.fsf
    now=`date`     
    echo "===> feat for own certainty has finished at $now" 
fi

if [[ $do_peerConfidence_masked == 1 ]]; then 
    feat $dir_designFiles/masked_groupComparison_peerConfidence.fsf
    now=`date`   
...
if [[ $do_perceived_certaintyE1_masked == 1 ]]; then 
    feat $dir_designFiles/masked_groupComparison_perceived_certaintyE1.fsf  
    now=`date`  
    echo "===> feat for perceived certainty at E1 has finished at $now" 
fi

if [[ $do_perceived_certaintySI_masked == 1 ]]; then 
    feat $dir_designFiles/masked_groupComparison_perceived_certaintySI.fsf  
    now=`date`  
    echo "===> feat for perceived certainty at SI has finished at $now" 
fi

if [[ $do_perceived_confidence_masked == 1 ]]; then 
    feat $dir_designFiles/masked_groupComparison_perceived_confidence.fsf  
    now=`date`  
...
#######################################################################################################
# Run group level feat, unmasked
#######################################################################################################


if [[ $do_perceived_certaintyE1 == 1 ]]; then 
    feat $dir_designFiles/groupComparison_perceived_certaintyE1.fsf  
    now=`date`  
    echo "===> feat for perceived certainty at E1 has finished at $now" 
fi
...
if [[ $do_perceived_confidence == 1 ]]; then 
    feat $dir_designFiles/groupComparison_perceived_confidence.fsf  
    now=`date`  
    echo "===> feat for perceived confidence at peer estimate has finished at $now" 
fi
