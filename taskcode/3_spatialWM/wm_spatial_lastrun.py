...
# start position of mouse cursor on every trial,
# (x, y) coordinates in degrees
# (the default is to place the cursor just below the
# target area)
MOUSE_START_POS = (
    0, -(AREA_HEIGHT_DEG/2 + 0.2)
)
...
if language == "English": 
    ...
    # (x, y) coordinates in degrees
    INSTRUCTIONS_CONTINUE_TXT_PLACEMENT = (0, -10)
    ...
    INSTRUCTIONS_TXT_1 = (
        "In dit spel zie je een aantal boxen.\n\n"
        "De boxen zullen in een bepaalde volgorde oplichten. "
        "Het is aan jou om de volgorde waarin ze oplichten te onthouden. "
        "Zodra ze stoppen met oplichten "
        "is het jouw taak om in diezelfde volgorde op de boxen te klikken.\n\n"
        "Gebruik de linker muisknop om op de boxen te klikken."
    )
    ...
    # "Click the boxes in the order that they lit up"
    PRACTICE_TXT = (
        "Klik op de boxen in de volgorde waarin ze oplichtten"
    )
    ...
    # when you are finished."
    INSTRUCTIONS_TXT_2 = (
        "Gedurende het spel zal het aantal boxen dat oplicht groter worden. "
        "Antwoord zo correct mogelijk.\n\n"
        "Het spel duurt ongeveer 5 tot 10 minuten. "
        "Je krijgt op het scherm te zien wanneer je klaar bent."
    )
...
expName = 'wmspatial'  # from the Builder filename that created this script
expInfo = {'participant': ''}
...
filename = _thisDir + os.sep + u'../../../raw_data/sub-%03d/spatialWM/sub-%03d_task-spatialWM_beh' % (int(expInfo['participant']),int(expInfo['participant']))

# An ExperimentHandler isn't essential but helps with data saving
thisExp = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath='Z:\\fMRI Projects\\fMRI Project Mushroom Highschool\\data_collection\\taskcode\\3_spatialWM\\wm_spatial_lastrun.py',
...
text_instructions_1 = visual.TextStim(win=win, name='text_instructions_1',
    ...
    font='Arial',
    units='deg', pos=(0, 0), height=TXT_SIZE_M, wrapWidth=50, ori=0, 
...
text_continue = visual.TextStim(win=win, name='text_continue',
    ...
    font='Arial',
    units='deg', pos=INSTRUCTIONS_CONTINUE_TXT_PLACEMENT, height=TXT_SIZE_S, wrapWidth=50, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=-2.0);
# overwrite date stamp for deidentification
expInfo['date'] = ""
...
text_end = visual.TextStim(win=win, name='text_end',
    text=END_TXT,
    font='Arial',
    units='deg', pos=(0, 0), height=TXT_SIZE_L, wrapWidth=50, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=0.0);
    ...
    while continueRoutine:
        ...
        if trial_phase == DEMO_TO_RESP_PHASE:
            ...
            if phase_switch_countdown <= 0:
                ...
                # move mouse cursor to start position
                mouse_trial.setPos(MOUSE_START_POS)
