...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='S:\\fMRI Projects\\fMRI Project Mushroom\\data_collection\\taskcode\\2_mushroomTask_confidence_MR\\mushroomTask.py',
...
for thisLoop_trial in loop_trials:
    ...
    while continueRoutine:
        ...
        if blue_box.status == STARTED:  # only update if drawing
            blue_box.setPos([markerPosition-0.026, -0.135])
        ...
        if showEstimate.status == STARTED:  # only update if drawing
            showEstimate.setPos([markerPosition-0.026, -0.135])
            showEstimate.setText(int(rating))
        ...
        if sliderMarker.status == STARTED:  # only update if drawing
            sliderMarker.setPos([markerPosition, -0.18])
        ...
        if red_box.status == STARTED:  # only update if drawing
            red_box.setPos([markerPosition+0.026, -0.225])
        ...
        if showEstimate_red.status == STARTED:  # only update if drawing
            showEstimate_red.setPos([markerPosition+0.026, -0.225])
            showEstimate_red.setText(100-int(rating))
    ...
    while continueRoutine:
        ...
        if showEstimate_E1.status == STARTED:  # only update if drawing
            showEstimate_E1.setPos([markerPosition_E1-0.026, -0.135])
        ...
        if blue_box_E2.status == STARTED:  # only update if drawing
            blue_box_E2.setPos([markerPosition_E2-0.026, -0.135])
        ...
        if showEstimate_E2.status == STARTED:  # only update if drawing
            showEstimate_E2.setPos([markerPosition_E2-0.026, -0.135])
            showEstimate_E2.setText(int(rating))
        ...
        if sliderMarker_E2.status == STARTED:  # only update if drawing
            sliderMarker_E2.setPos([markerPosition_E2, -0.18])
        ...
        if enterToRespond.status == STARTED:  # only update if drawing
            enterToRespond.setText(howToRespond)
        ...
        if showEstimate_red_E1.status == STARTED:  # only update if drawing
            showEstimate_red_E1.setPos([markerPosition_E1+0.026, -0.225])
        ...
        if red_box_E2.status == STARTED:  # only update if drawing
            red_box_E2.setPos([markerPosition_E2+0.026, -0.225])
        ...
        if showEstimate_red_E2.status == STARTED:  # only update if drawing
            showEstimate_red_E2.setPos([markerPosition_E2+0.026, -0.225])
            showEstimate_red_E2.setText(100-int(rating))
