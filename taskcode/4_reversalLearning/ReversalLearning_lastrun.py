...
filename = _thisDir + os.sep + u'../../../raw_data/sub-%03d/reversalLearning/sub-%03d_task-reversalLearning_beh' % (int(expInfo['participant']),int(expInfo['participant']))

# An ExperimentHandler isn't essential but helps with data saving
thisExp = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath='Z:\\fMRI Projects\\fMRI Project Mushroom Highschool\\data_collection\\taskcode\\4_reversalLearning\\ReversalLearning_lastrun.py',
...
circle_example = visual.ShapeStim(
    win=win, name='circle_example', vertices=1000,
    size=(0.15, 0.15),
    ori=0.0, pos=(0.15, -0.3),
    lineWidth=15.0,     colorSpace='rgb',  lineColor='black', fillColor=None,
...
response_button = keyboard.Keyboard()
...
trial = 1
nTrials = 250
nTrials_for_bonus = 20 # how many trials are selected to base bonus points on? 
...
circle = visual.ShapeStim(
    win=win, name='circle', vertices=1000,
    size=(0.15, 0.15),
    ori=0.0, pos=[0,0],
    lineWidth=15.0,     colorSpace='rgb',  lineColor='black', fillColor=None,
    opacity=None, depth=-5.0, interpolate=True)
...
for thisLoop_instruction in loop_instructions:
    ...
    for thisComponent in instructionsComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    if (navigate.keys == "return" or navigate.keys == "num_enter") and int(currentInstr) == 6:
...
for thisLoop_trial in loop_trials:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    response_button.keys = []
    response_button.rt = []
    _response_button_allKeys = []
    # randomize lateral position of triangle and circle
    ...
    # keep track of which components have finished
    trialComponents = [response_button, background_triangle, triangle, background_circle, circle]
    ...
    while continueRoutine:
        ...
        # *response_button* updates
        waitOnFlip = False
        if response_button.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            response_button.frameNStart = frameN  # exact frame index
            response_button.tStart = t  # local t and not account for scr refresh
            response_button.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(response_button, 'tStartRefresh')  # time at next scr refresh
            response_button.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(response_button.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(response_button.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if response_button.status == STARTED and not waitOnFlip:
            theseKeys = response_button.getKeys(keyList=['left', 'right'], waitRelease=False)
            _response_button_allKeys.extend(theseKeys)
            if len(_response_button_allKeys):
                response_button.keys = _response_button_allKeys[-1].name  # just the last key pressed
                response_button.rt = _response_button_allKeys[-1].rt
                # a response ends the routine
                continueRoutine = False
...
continueRoutine = True
# update component parameters for each repeat
# save now to compare total points with previous studies
...
# Experimenter closes task by pressing "s"
